...
allowed-tools: Bash(node *scripts/preflight.mjs*), Bash(node *scripts/flow-check.mjs*), Bash(node *scripts/dc-to-html.mjs*), Bash(git status *), Bash(ls *)
---
...
4. Copy any exported PNGs the designer placed in `design/flows/<slug>/png/` to `./specs/<title>/design/png/`.
...
```
Design: ./specs/<title>/design-flow.md (canvas <url>, v<n>). Screen references: ./specs/<title>/design/*.html
```
