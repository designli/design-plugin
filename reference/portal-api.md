...
- Sync: `GET /projects/:p/flows/:f/head` → pull comments and text edits (`?status=all`, follow `nextCursor`) → `POST /projects/:p/flows/:f/versions` with `If-Match: <head version>` (0 for a new flow) and `X-Designli-Last-Pull: <serverTime of the pull>`; 409 `STALE_LOCAL` means pull first, `?force=1` overrides; 200 `reused: true` for an identical content hash (journey `order`/`next` still updated).
