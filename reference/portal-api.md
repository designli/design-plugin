...
- **Device sign-in** (how the plugin gets its token without anyone copying it): `POST /device/start {label?, projects?, permissions?, expiresInDays?, client?}` (public) → `201 {deviceCode (ddev_… secret), userCode (WXYZ-2345), verificationUrl, verificationUrlComplete, expiresIn: 600, interval: 5}`. The person opens `verificationUrlComplete` signed in as staff and approves or denies (`GET /device/{userCode}`, `POST /device/{userCode}/approve {scope overrides}`, `POST /device/{userCode}/deny`; browser session only, a token cannot approve). The requester polls `POST /device/poll {deviceCode}` every `interval` seconds: `{status: "pending"}`, then once `{status: "approved", token, ...token row}` (minted at that moment with the approver as owner, never wider than their access) or `{status: "denied" | "expired"}`; afterwards `404`.
- Token lookup in `portal.mjs`: `DESIGNLI_PORTAL_TOKEN`, then `~/.config/designli-design/credentials.json` (0600, written by the device sign-in, or by `setup.mjs --paste` / a piped `portal.mjs login`). Never a command-line flag, never in a repo.
