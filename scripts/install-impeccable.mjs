...
import { existsSync, mkdirSync, readFileSync, writeFileSync, readdirSync, statSync, rmSync, cpSync, appendFileSync } from "node:fs";
...
export function checkInstall(project) {
  ...
  const installedVersion = (readFileSync(join(skillDir, "SKILL.md"), "utf8").match(/^version:\s*(\S+)/m) || [])[1] || "unknown";
  if (installedVersion !== IMPECCABLE_PIN) return { status: "VERSION_MISMATCH", installed: installedVersion, pinned: IMPECCABLE_PIN };
  const drift = [];
  for (const [rel, hash] of Object.entries(pin.files)) {
    ...
    const target = rel.startsWith("skills/impeccable/")
      ...
      : rel.startsWith("agents/") ? join(agentsDir, rel.slice("agents/".length)) : null;
    if (!target) continue;
    if (!existsSync(target)) { drift.push({ file: rel, reason: "missing" }); continue; }
...
function mergeSettings(project) {
  ...
  let s = {};
  if (existsSync(file)) { try { s = JSON.parse(readFileSync(file, "utf8")); } catch { throw new Error(`${file} is not valid JSON; fix it before installing`); } }
...
export function install(project, { force = false } = {}) {
  ...
  if (force || before.status !== "OK") {
    ...
    for (const f of readdirSync(join(VENDOR, "agents"))) cpSync(join(VENDOR, "agents", f), join(agentsDir, f));
    const pin = JSON.parse(readFileSync(join(VENDOR, "PIN.json"), "utf8"));
    writeFileSync(join(skillDir, ".designli-pin.json"), JSON.stringify({ skillVersion: pin.skillVersion, upstreamCommit: pin.upstreamCommit, installedAt: new Date().toISOString() }, null, 2) + "\n");
    written.push(relative(project, skillDir) + "/", relative(project, agentsDir) + "/impeccable-*.md");
  }
  if (ensureBlock(join(project, ".gitignore"), GITIGNORE_BLOCK, "designli-design plugin")) written.push(".gitignore");
  if (ensureBlock(join(project, ".prettierignore"), PRETTIERIGNORE_BLOCK, "designli-design plugin")) written.push(".prettierignore");
  mergeSettings(project); written.push(".claude/settings.json");
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (major < 22 || (major === 22 && minor < 12)) {
    console.error(`Node ${process.versions.node} is too old; impeccable needs >= 22.12 (24 recommended: nvm install 24)`);
    process.exit(1);
  }
  if (args.includes("--check")) {
    const r = checkInstall(project);
    console.log(json ? JSON.stringify(r) : `impeccable ${r.status}${r.installed ? ` installed=${r.installed}` : ""} pinned=${r.pinned}${r.drift ? ` (${r.drift.length} files: ${r.drift.slice(0, 5).map(d => d.file).join(", ")})` : ""}`);
    process.exit(r.status === "OK" ? 0 : 1);
  }
  const r = install(project, { force: args.includes("--force") });
  console.log(json ? JSON.stringify(r) : `impeccable ${r.before} -> ${r.after.status} (${IMPECCABLE_PIN}); wrote: ${r.written.join(", ")}`);
