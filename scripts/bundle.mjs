...
if (!srcDir) {
  console.error("need --flow <dir> (or --components <dir> --kind components)");
  process.exit(2);
}
if (!existsSync(srcDir) || !readdirSync(srcDir).some((f) => f.endsWith(".dc.html"))) {
  console.error(`no .dc.html sources in ${srcDir}`);
  process.exit(2);
}
...
export function parseStem(stem) {
  if (stem === "Main") return { n: "", stepId: "", state: "", device: "desktop", id: "Main" };
...
const r = spawnSync(
  process.execPath,
  [new URL("./dc-to-html.mjs", import.meta.url).pathname, ...dcArgs],
  { encoding: "utf8" },
);
if (r.status !== 0) {
...
if (
  !existsSync(join(out, "screens")) ||
  !readdirSync(join(out, "screens")).some((f) => f.endsWith(".html"))
) {
  console.error("no screens were produced");
  process.exit(1);
}
const warnings = r.stdout.split("\n").filter((l) => /problem/.test(l));

// 2. hash + manifest
const screenFiles = readdirSync(join(out, "screens"))
  .filter((f) => f.endsWith(".html"))
  .sort();
const entries = screenFiles.map((f) => ({
  path: `screens/${f}`,
  sha256: sha(readFileSync(join(out, "screens", f))),
}));
const contentHash = contentHashOf(entries);
const canvas = existsSync(join(srcDir, "canvas.json"))
  ? JSON.parse(readFileSync(join(srcDir, "canvas.json"), "utf8"))
  : { artboards: [] };
const dims = Object.fromEntries(
  (canvas.artboards || []).map((a) => [a.file.replace(/\.dc\.html$/, ""), a]),
);
let manifest;
if (kind === "flow") {
  const flow = JSON.parse(readFileSync(join(srcDir, "flow.json"), "utf8"));
  const devices = { desktop: flow.frame || { w: 1440, h: 900 } };
  if ((flow.devices || []).includes("mobile"))
    devices.mobile = flow.mobileFrame || { w: 390, h: 844 };
  ...
  for (const f of screenFiles) {
    ...
    if (!p) {
      warnings.push(`skipped ${f}: not a screen stem`);
      continue;
    }
    const e = screens.get(p.id) || {
      id: p.id,
      kind: p.id === "Main" ? "map" : "state",
      title: p.id === "Main" ? "Flow map" : `${p.n} ${p.stepId} · ${p.state}`,
      step: p.n || null,
      stepId: p.stepId || null,
      state: p.state || null,
      interactive: interactive.has(p.id),
      devices: {},
    };
    const a = dims[stem] || {};
    e.devices[p.device] = {
      file: `screens/${f}`,
      w: a.w || (p.device === "mobile" ? devices.mobile?.w : devices.desktop.w) || 1440,
      h: a.h || 900,
      sha256: "sha256:" + entries.find((x) => x.path === `screens/${f}`).sha256,
      source: `${stem}.dc.html`,
  ...
  const rank = (s) =>
    s.id === "Main"
      ? "0"
      : `${s.step}-${String(STATE_ORDER.indexOf(s.state) < 0 ? 99 : STATE_ORDER.indexOf(s.state)).padStart(2, "0")}`;
  const steps = (flow.steps || []).map((s) => ({
    ...
    states: Object.entries(s.states || {}).map(([state, v]) =>
      /^n\/a/.test(String(v))
        ? { state, waived: String(v).replace(/^n\/a:\s*/, "") }
        : { state, screen: `${s.n}-${s.id}-${state}` },
    ),
  }));
  const compHash =
    compDir && existsSync(join(compDir, "bundle", "manifest.json"))
      ? JSON.parse(readFileSync(join(compDir, "bundle", "manifest.json"), "utf8")).contentHash
      : null;
  manifest = {
    schema: 1,
    generator: `designli-design/${PLUGIN_VERSION}`,
  ...
  const screens = screenFiles.map((f) => {
    ...
    return {
      id: stem,
      kind: "state",
      title: a.title || stem.replace(/^Cmp/, ""),
      step: null,
      stepId: null,
      state: null,
      interactive: false,
...
if (kind === "flow") {
  const fj = join(srcDir, "flow.json");
  const flow = JSON.parse(readFileSync(fj, "utf8"));
  flow.bundle = { builtAt: manifest.generatedAt, contentHash, screens: manifest.screens.length };
  writeFileSync(fj, JSON.stringify(flow, null, 2) + "\n");
}
...
if (bytes > 10 * 1024 * 1024)
  res.warnings.push(`bundle is ${(bytes / 1048576).toFixed(1)} MB; the portal refuses above 20 MB`);
console.log(
  json
    ? JSON.stringify(res)
    : `bundle: ${res.kind} ${res.screens} screens (${res.files} files, ${(bytes / 1024).toFixed(0)} KB) -> ${res.out}  ${contentHash}${warnings.length ? "\n  " + warnings.join("\n  ") : ""}`,
);
