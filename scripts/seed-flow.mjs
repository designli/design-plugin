...
if (!skillDir || !flowDir || !out || !title) {
  console.error("need --skill-dir, --flow, --out, --title");
  process.exit(2);
}
const helper = join(skillDir, "seed-canvas.mjs"),
  template = join(skillDir, "payload.template.html");
if (!existsSync(helper) || !existsSync(template)) {
  console.error(
    `design skill helper not found in ${skillDir}; invoke /design once in this session to learn the base directory`,
  );
  process.exit(2);
}

const flowBoards = readdirSync(flowDir)
  .filter((f) => f.endsWith(".dc.html"))
  .sort();
const compBoards =
  compDir && compDir !== flowDir
    ? readdirSync(compDir)
        .filter((f) => /^Cmp[A-Za-z0-9]+\.dc\.html$/.test(f))
        .sort()
    : [];
const artboards = [
  ...flowBoards.map((f) => join(flowDir, f)),
  ...compBoards.map((f) => join(compDir, f)),
];
const images = readdirSync(flowDir)
...
if (canvasPath && compBoards.length) {
  ...
  if (!c.pages.some((p) => p.id === "components"))
    c.pages.push({ id: "components", name: "Components" });
  ...
  for (const f of compBoards) {
    ...
    c.artboards.push({
      ...
      h: 720,
      page: "components",
      title: f.replace(/\.dc\.html$/, "").replace(/^Cmp/, ""),
    });
    x += 960 + 120;
  }
  ...
  const seedDir = join(flowDir, ".seed");
  mkdirSync(seedDir, { recursive: true });
  canvasPath = join(seedDir, "canvas.json");
  writeFileSync(canvasPath, JSON.stringify(c, null, 2) + "\n");
}
...
console.log(
  `seed-flow: ${artboards.length} artboards (${compBoards.length} components), ${images.length} images -> ${relative(process.cwd(), out)}`,
);
