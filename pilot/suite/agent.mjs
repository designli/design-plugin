...
if (!PORTAL || !PROJECT || !TOKEN_FILE) {
  console.error(
    "usage: node agent.mjs --portal <url> --project <id> --designer-token-file <file> [--client-token-file <file>] [--model <m>] [--keep]",
  );
  process.exit(2);
}
const designer = Token.fromFile(TOKEN_FILE, "designer");
const client = has("--client-token-file")
  ? Token.fromFile(opt("--client-token-file"), "client")
  : null;
...
const RESULTS = resolve(
  import.meta.dirname,
  "results",
  `${stamp}-tier2-${new URL(PORTAL).hostname}`,
);
...
async function step(name, prompt, { maxTurns = 80 } = {}) {
  ...
  const args = [
    ...
    "dontAsk",
    "--allowedTools",
    "mcp__plugin_designli-design_designli-design__*,mcp__designli-design__*,Bash(node *),Bash(git *),Bash(ls *),Bash(cat *),Read,Edit,Write,Glob,Grep,MultiEdit",
    "--append-system-prompt",
    SYSTEM,
    "--max-turns",
    String(maxTurns),
    "--settings",
    JSON.stringify({ enabledPlugins: { "designli-design@designli-tools": false } }),
  ];
  ...
  const r = await new Promise((res) => {
    const p = spawn("claude", args, {
      cwd: REPO,
      env: { ...process.env, DESIGNLI_PORTAL_TOKEN: designer.value, DESIGNLI_PORTAL_URL: PORTAL },
  ...
  const questions = (text.split(/QUESTIONS I WOULD HAVE ASKED/i)[1] ?? "")
    .split("\n")
  ...
  const entry = {
    ...
    tools: tools.map((t) => t.name.replace(/^mcp__plugin_designli-design_designli-design__/, "")),
    forced: tools.some((t) => /publish$/.test(t.name) && t.input?.force === true),
    tokenInText: /dpat_[A-Za-z0-9_-]{8,}/.test(r.out),
    questions,
    resultTail: text.slice(-600),
    stderr: r.err.slice(0, 300),
  };
  obs.agent.steps.push(entry);
  writeFileSync(
    join(RESULTS, `transcript-${name}.jsonl`),
    r.out.replace(/dpat_[A-Za-z0-9_-]{8,}/g, "dpat_…"),
  );
  say(
    `  ${name}: exit ${r.code}, ${entry.turns} turns, $${entry.costUsd ?? "?"}, ${tools.length} tool calls, ${questions.length} questions listed`,
  );
  return entry;
}
...
await step(
  "setup",
  `/designli-design:setup Portal ${PORTAL}, project ${PROJECT}, harness Claude Code. The token is already in the environment.`,
);
await step("adopt", "/designli-design:adopt");
await step("publish1", '/designli-design:publish "first cut"');
sh(
  "git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm agent-adopt -q || true; git push -q",
);
...
if (client) {
  ...
  if (su?.manifest) {
    ...
    await api.post(client, `${flowOf("sign-up")}/comments`, {
      text: "The button says Continue twice; and can the email field explain the format?",
      screen: { id: scr.id, device: "desktop" },
      anchor: { x: 40, y: 40 },
      flowVersion: su.number,
    });
    const edit = (screen, elementPath, originalText, newText) =>
      api.post(client, `${flowOf("sign-up")}/text-edits`, {
        screen,
        elementPath,
        originalText,
  ...
  await api.post(client, `${flowOf("sign-up")}/comments`, {
    ...
    flowVersion: su?.number ?? null,
  });
  const stale = await step("stale", 'Publish "round 3" now without pulling feedback first.');
  obs.agent.staleAttempt = {
    forced: stale.forced,
    publishedAnyway:
      /release (\d+)|published/i.test(stale.resultTail) && !/refus|pull/i.test(stale.resultTail),
  };
}
await step("handoff", '/designli-design:handoff buy-tickets "Buy tickets"');
sh(
  "git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm agent-end -q || true; git push -q",
);
...
for (const k of key.flows) {
  ...
  const devicesOf = (flow) =>
    flow.devices?.length
      ? flow.devices
      : ["desktop", "mobile"].filter((d) =>
          (flow.steps ?? []).some((st) =>
            Object.values(st.states ?? {}).some(
              (v) =>
                (typeof v === "string" && d === "desktop") || (v && typeof v === "object" && v[d]),
            ),
          ),
        );
  obs.adopt.proposed.push({
    slug: k.slug,
    devices: devicesOf(f),
...
for (const st of obs.agent.steps)
  for (const q of st.questions)
    obs.adopt.questions.push({
      flow: q.split(":")[0].trim(),
      field: (q.split(":")[1] ?? "").trim() || "unknown",
      text: q,
    });
{
  const { McpClient } = await import("./lib/rpc.mjs");
  const c = new McpClient(REPO, {
    env: { DESIGNLI_PORTAL_TOKEN: designer.value, DESIGNLI_PORTAL_URL: PORTAL },
  });
  ...
  obs.reliability = {
    status: { ok: st.out?.ok ?? null, pluginNotice: st.out?.plugin?.message ?? null },
  };
  await c.close();
}
...
obs.publish1 = {
  ok: flows.some((f) => f.latestVersion > 0),
  pushed: flows.filter((f) => f.latestVersion > 0).map((f) => f.id),
  ms: obs.agent.steps.find((s) => s.name === "publish1")?.ms ?? null,
  http429: 0,
};
obs.portal1 = { flows: [] };
...
obs.portal1.components = cm.json?.screens
  ? { sheets: cm.json.screens.map((s) => ({ id: s.id, styled: true })) }
  : { error: cm.error };
const countNew = (text) =>
  Number(
    sh(
...
obs.feedback = {
  filesWith: { "Help center": countNew("Help center"), "Your email": countNew("Your email") },
  includeEditedOnce:
    existsSync(join(REPO, "design/components/Header.html")) &&
    readFileSync(join(REPO, "design/components/Header.html"), "utf8").includes("Help center") &&
    countNew("Help center") === 1,
};
const hs =
  (await api.get(designer, `/projects/${PROJECT}/flows/buy-tickets/handoffs`)).json?.handoffs ?? [];
obs.handoff = { flows: [{ slug: "buy-tickets", ok: hs.length > 0 }] };
obs.agent.totals = {
  turns: obs.agent.steps.reduce((n, s) => n + (s.turns ?? 0), 0),
  costUsd: Math.round(obs.agent.steps.reduce((n, s) => n + (s.costUsd ?? 0), 0) * 100) / 100,
  questions: obs.agent.steps.reduce((n, s) => n + s.questions.length, 0),
  forced: obs.agent.steps.some((s) => s.forced),
  tokenLeaked: obs.agent.steps.some((s) => s.tokenInText),
};
