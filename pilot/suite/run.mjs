...
if (!PORTAL) {
  console.error(
    "usage: node run.mjs --portal <url> [--project marquee] [--repo <dir>] [--keep] [--dev-admin] [--skip a,b]",
  );
  process.exit(2);
}
const PROJECT = opt(
  "--project",
  `marquee-${new Date().toISOString().slice(2, 16).replace(/[-T:]/g, "")}`,
);
...
const RESULTS = resolve(
  import.meta.dirname,
  "results",
  `${stamp}-tier1${process.argv.includes("--corpus") && process.argv[process.argv.indexOf("--corpus") + 1] === "xl" ? "-xl" : ""}-${new URL(PORTAL).hostname}`,
);
...
const obs = {
  meta: {
    ...
    plugin: JSON.parse(readFileSync(join(PLUGIN_ROOT, ".claude-plugin", "plugin.json"), "utf8"))
      .version,
  },
  errors: [],
  timings: {},
...
const fail = (step, e) => {
  const entry = {
    ...
    message: String(e?.message ?? e?.error?.message ?? e).slice(0, 400),
    run: e?.run ?? e?.error?.run ?? null,
  };
...
const writeCreds = (token) =>
  writeFileSync(
    ...
    JSON.stringify({
      portals: { [PORTAL]: { token: token.value, savedAt: new Date().toISOString() } },
    }),
    { mode: 0o600 },
  );
...
const mcp = (project = REPO) =>
  new McpClient(project, { env: { HOME, DESIGNLI_PORTAL_TOKEN: "", DESIGNLI_PORTAL_URL: PORTAL } });
...
sh(
  "git init -q -b main && git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm corpus",
);
...
await timed("signin", async () => {
  if (has("--dev-admin")) {
    const r = await fetch(`${PORTAL}/api/v1/auth/dev`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ email: "admin@designli.co", name: "Suite Admin" }),
    });
    ...
    for (let cycle = 0; cycle < 6; cycle++) {
      ...
      obs.signin = {
        mode: "device",
        userCode: s.out.userCode,
        leaked: JSON.stringify(s.out).includes("ddev_"),
        cycles: cycle + 1,
      };
    ...
    if (p.out?.status !== "approved")
      throw new Error(`sign-in ${p.out?.status ?? p.error?.message}`);
    obs.signin.ok = true;
    obs.signin.leaked = obs.signin.leaked || JSON.stringify(p.out).includes("dpat_");
    const cred = JSON.parse(
      readFileSync(join(HOME, ".config", "designli-design", "credentials.json"), "utf8"),
    );
...
await timed("project", async () => {
  ...
  obs.project = {
    created: pr.ok,
    error: pr.error ?? null,
    visible: pr.out?.projects?.some((p) => p.id === PROJECT) ?? null,
  };
  await c.close();
  const session = await api.session(admin);
  designer = await api.mint(session, {
    label: "suite-designer",
    projects: [PROJECT],
    permissions: ["view", "comment", "suggest_copy", "push", "resolve", "manage_flows"],
    expiresInDays: 30,
  });
  client = await api.mint(session, {
    label: "suite-client",
    projects: [PROJECT],
    permissions: ["view", "comment", "suggest_copy"],
    expiresInDays: 30,
  });
  dev = await api.mint(session, {
    label: "suite-dev",
    projects: [PROJECT],
...
await timed("setup", async () => {
  ...
  obs.setup = {
    ok: s.ok,
    written: s.out?.written ?? null,
    literalToken: /dpat_[A-Za-z0-9_-]{8,}/.test(mcpJson),
    envExpansion: mcpJson.includes("${DESIGNLI_PORTAL_TOKEN}"),
    nextSteps: s.out?.nextSteps ?? null,
  };
  const cs = await tool(c, "credentials_status", { url: PORTAL });
  obs.setup.credentials = {
    ok: cs.out?.ok ?? false,
    scopeProjects: cs.out?.scope?.projects ?? null,
    leaked: JSON.stringify(cs.out ?? {}).includes("dpat_"),
  };
  await c.close();
});
...
await timed("adopt", async () => {
  ...
  obs.adopt = {
    scan: {
      screens: scan.out?.screens?.length ?? null,
      components: (scan.out?.components ?? []).map((x) => ({
        name: x.name,
        usedBy: x.usedBy.length,
      })),
    },
    proposed: (prop.out?.flows ?? []).map((f) => ({
      slug: f.slug,
      devices: f.devices,
      title: f.title,
      entry: f.entryPoints?.map((e) => e.to) ?? [],
      steps: f.steps.map((s) => ({
        n: s.n,
        id: s.id,
        kind: s.kind,
    ...
    questions: (prop.out?.questions ?? []).map((q) => ({
      flow: q.flow,
      field: q.field,
  ...
  obs.adopt.gaps = (g.out?.gaps ?? []).map((x) => ({
    flow: x.flow,
    kind: x.kind,
    where: x.where,
    message: x.message,
...
function FLOW_GOAL(slug) {
  return (
    { "sign-up": "A fan creates an account with an email and a name." }[slug] ?? `Goal of ${slug}.`
  );
}
...
await timed("publish1", async () => {
  ...
  obs.publish1 = {
    firstAttempt: {
      ...
      message: (p.error?.message ?? "").slice(0, 160),
      pushed: p.out?.pushed?.length ?? 0,
    },
  };
  if (!p.ok && /include Nope not found/.test(p.error?.message ?? "")) {
    ...
    for (const f of readdirSync(dir).filter((x) => x.endsWith(".html")))
      writeFileSync(
        join(dir, f),
        readFileSync(join(dir, f), "utf8").replace(/<dc-import name="Nope"><\/dc-import>\n?/g, ""),
      );
    ...
    if (again.ok)
      p = {
        ...again,
        out: { ...again.out, pushed: [...(p.out.pushed ?? []), ...(again.out.pushed ?? [])] },
      };
  }
  if (ROUNDS)
    obs.xl = {
      flows: key.flows.length,
      screens: key.flows.reduce((n, f) => n + f.files, 0),
      publishMs: p.ms,
      secondsPerFlow: Math.round(p.ms / key.flows.length / 100) / 10,
    };
  Object.assign(obs.publish1, {
    ok: p.ok,
    ms: p.ms,
    pushed: p.out?.pushed?.map((x) => x.flow) ?? [],
    unchanged: p.out?.unchanged?.map((x) => x.flow) ?? [],
    components: p.out?.components ?? null,
  ...
  void before;
  sh(
    "git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm publish1 -q || true; git push -q",
  );
});
...
await timed("portal1", async () => {
  ...
  for (const f of flows) {
    ...
    obs.portal1.flows.push({
      slug: f.id,
      version: v.json.number,
      devices: Object.keys(m.devices),
      screens: m.screens.length,
      mobileScreens: m.screens.filter((s) => s.devices.mobile).length,
      desktopScreens: m.screens.filter((s) => s.devices.desktop).length,
      overlaps: overlaps(m),
  ...
  if (cm.json?.screens) {
    ...
    for (const s of cm.json.screens) {
      const h = await api.get(
        designer,
        `/p/${PROJECT}/components/latest/${s.devices.desktop.file}?raw=1`,
        { raw: true },
      );
      sheets.push({
        id: s.id,
        w: s.devices.desktop.w,
        h: s.devices.desktop.h,
        styled: /--accent/.test(h.text),
        fonts: /<link[^>]*stylesheet|<style/.test(h.text),
      });
    }
  ...
  if (bt) {
    const raw = await api.get(
      designer,
      `/p/${PROJECT}/buy-tickets/v${bt.version}/screens/01-Tickets-Default.html?raw=1`,
      { raw: true },
    );
    obs.portal1.nested = {
      unresolvedImports: (raw.text.match(/<dc-import/g) || []).length,
      logo: /class="mq-logo">Marquee</.test(raw.text),
    };
  }
  // a screen fetched twice: ETag and 304
  const f0 = obs.portal1.flows.find((x) => x.version);
  if (f0) {
    const m = (await api.get(designer, `/projects/${PROJECT}/flows/${f0.slug}/versions/latest`))
      .json.manifest;
    const file = m.screens.find((s) => s.devices.desktop)?.devices.desktop.file;
    const r1 = await api.get(designer, `/p/${PROJECT}/${f0.slug}/v${f0.version}/${file}`, {
      raw: true,
    });
    const et = r1.headers.get("etag");
    const r2 = await api.get(designer, `/p/${PROJECT}/${f0.slug}/v${f0.version}/${file}`, {
      raw: true,
      headers: { "if-none-match": et ?? "" },
    });
    obs.portal1.caching = {
      etag: !!et,
      notModified: r2.status === 304,
      bridge: r1.text.includes("data-dp-bridge"),
    };
  }
});
  ...
  for (const f of readdirSync(join(d, "design", "flows"))) {
      ...
      Array.isArray(v)
        ...
        : v && typeof v === "object"
          ? "{" +
            Object.keys(v)
              .sort()
              .map((x) => JSON.stringify(x) + ":" + stable(v[x]))
              .join(",") +
            "}"
    ...
    for (const k of keys)
      if (stable(before[k] ?? null) !== stable(after[k] ?? null)) {
        ...
        if (out.length <= 3)
          out.push(
            `  before ${JSON.stringify(before[k]).slice(0, 220)}`,
            `  after  ${JSON.stringify(after[k]).slice(0, 220)}`,
          );
      }
  }
  return out;
...
if (!SKIP.has("lost"))
  await timed("lostRepo", async () => {
    ...
    for (const f of readdirSync(join(d, "design", "flows")))
      rmSync(join(d, "design", "flows", f, "flow.json"), { force: true });
    ...
    obs.lostRepo = {
      adopt: {
        ok: a.ok,
        flows: a.out?.flows?.length ?? a.out?.written?.length ?? null,
        notRebuilt: a.out?.notRebuilt ?? [],
      },
      dry: statuses,
      dryByFlow: (dry.out?.flows ?? []).map((f) => ({ flow: f.flow, status: f.status })),
      unchanged: statuses.filter((s) => s === "unchanged").length,
      of: statuses.length,
      differing: flowJsonDiffs(d),
    };
    rmSync(d, { recursive: true, force: true });
  });
...
if (!SKIP.has("stress"))
  await timed("stress", async () => {
    ...
    for (const f of readdirSync(join(dir, "design", "flows")))
      if (!f.startsWith("stress-")) rmSync(join(dir, "design", "flows", f), { recursive: true });
    ...
    for (const slug of ["stress-files", "stress-bytes"]) {
      ...
      const msg = [
        r.error?.message ?? "",
        ...tooLarge
          .filter((x) => x.flow === slug || String(x.where).includes(`/${slug}/`))
          .map((x) => x.proposal),
      ].join(" ");
      obs.stress[slug] = {
        ok: r.ok,
        ...
        ms: Date.now() - t0,
        hasFix: /400 per flow|split the flow|20 MB|trim|link it by URL|not scanned/i.test(msg),
      };
    }
    await c.close();
    const flows = (await api.get(designer, `/projects/${PROJECT}/flows`)).json?.flows ?? [];
    obs.stress.leakedFlows = flows
      .filter((f) => f.id.startsWith("stress-") && f.latestVersion > 0)
      .map((f) => f.id);
    rmSync(dir, { recursive: true, force: true });
  });
...
if (ROUNDS) {
  await timed("handoffV1", async () => {
    ...
    for (const slug of ["buy-tickets", "sign-up", "payouts"]) {
      const h = await c.call("handoff", {
        flow: slug,
        story: `As a user I can ${slug.replace(/-/g, " ")}`,
      });
      obs.handoff.flows.push({
        slug,
        ok: h.ok,
        error: h.error ? `${h.error.code}: ${String(h.error.message).slice(0, 120)}` : null,
      });
    }
    // reports carries a too-large screen: the handoff must refuse rather than hand off a gap
    const rpt = await c.call("handoff", {
      flow: "reports",
      story: "As an organizer I can export reports",
    });
  ...
  roundsUi = await runRounds({
    ...
    timed,
    secretFile,
    tokens: { admin, designer, client, dev },
  });
}
// ---- 4. the client round ----
if (!ROUNDS)
  await timed("clientRound", async () => {
    ...
    const out = await new Promise((res) => {
      const p = spawn(
        process.execPath,
        [
          join(PLUGIN_ROOT, "pilot", "lab", "client-round.mjs"),
          "--url",
          PORTAL,
          "--project",
          PROJECT,
    ...
    obs.clientRound = {
      lab: lab
        ? {
            ...
            edits: lab.edits.length,
            waiver: lab.waiver,
            reorder: !!lab.reorder,
            agentThread: lab.threads.find((t) => t.sentToAgent)?.id ?? null,
          }
        : null,
      nasty: {},
    };
    ...
    const r1 = await comment("sign-up", {
      text: '<script>alert("x")</script> Looks good 🎟️ — but the button says "Continue" twice?',
      screen: { id: scr.id, device: "desktop" },
      anchor: { x: 40, y: 40 },
      flowVersion: su.number,
    });
    n.scriptComment = {
      status: r1.status,
      storedVerbatim: r1.json?.text?.includes("<script>") ?? null,
      id: r1.json?.id ?? null,
    };
    ...
    const edit = (slug, v, screen, elementPath, originalText, newText) =>
      api.post(client, `${flowOf(slug)}/text-edits`, {
        screen,
        elementPath,
        originalText,
    ...
    n.edits = {
      include: { status: e1.status, id: e1.json?.id },
      screen: { status: e2.status, id: e2.json?.id },
      supersede: { status: e3.status, replaced: e3.json?.replaced ?? null, id: e3.json?.id },
    };
    ...
    const e5 = await edit(
      "sign-up",
      su.number,
      { id: wscr.id, device: "desktop" },
      "b/1/2/0",
      "Back to events",
      "Back to shows",
    );
    n.edits.toDismiss = { status: e5.status, id: e5.json?.id };
    const w = await api.post(client, `${flowOf("request-a-refund")}/waivers`, {
      step: "02",
      state: "Error",
      reason: "client says never",
    });
    n.clientWaiver = { status: w.status, code: w.code };
    const push = await api.post(
      client,
      `${flowOf("sign-up")}/versions`,
      { manifest: {}, files: [] },
      { headers: { "if-match": "1" } },
    );
    n.clientPush = { status: push.status, code: push.code };
    const staffWaive = await api.post(designer, `${flowOf("request-a-refund")}/waivers`, {
      step: "02",
      state: "Error",
      reason: "refunds are approved by hand: no server error path in the pilot",
    });
    n.staffWaiver = { status: staffWaive.status };
    if (r1.json?.id) {
      const flag = await api.patch(designer, `${flowOf("sign-up")}/comments/${r1.json.id}`, {
        sentToAgent: true,
      });
      n.sentToAgent = { status: flag.status, id: r1.json.id };
      const clientFlag = await api.patch(client, `${flowOf("sign-up")}/comments/${r1.json.id}`, {
        sentToAgent: false,
      });
  ...
  await timed("feedback", async () => {
    ...
    obs.feedback = {
      pull: {
        ok: pull.ok,
        flows:
          pull.out?.flows?.map((f) => ({
            flow: f.flow ?? f.slug,
            threads: f.threads ?? f.comments ?? null,
            edits: f.edits ?? f.textEdits ?? null,
          })) ?? pull.out,
      },
      apply: {},
      dismiss: null,
    ...
    if (toDismiss) {
      const d = await c.call("edits_dismiss", {
        flow: "sign-up",
        id: toDismiss,
        reason: "the label matches the button on the events page; keeping it",
      });
    ...
    for (const slug of ["sign-up", "transfer-a-ticket"]) {
      ...
      obs.feedback.apply[slug] = {
        ok: a.ok,
        applied: a.out?.applied ?? null,
        needsManual: a.out?.needsManual?.length ?? null,
        sourceFiles: (a.out?.results ?? []).reduce(
          (n, r) => n + (r.files ?? []).filter((f) => f.result === "applied" && !f.include).length,
          0,
        ),
        includeHits: (a.out?.results ?? []).flatMap((r) =>
          (r.files ?? [])
            .filter((f) => f.include)
            .map((f) => ({
              include: f.include,
        ...
        files: (a.out?.results ?? []).map((r) => ({
          id: r.id,
          result: r.result,
          files: (r.files ?? []).filter((f) => f.result === "applied").length,
        })),
        error: a.error ?? null,
      };
    }
    obs.feedback.filesWith = {
      "Help center": Number(countNew("Help center")),
      Email: Number(
        sh(`grep -rl --exclude-dir=bundle --include='*.html' -- '>Email<' design | wc -l`),
      ),
      "Your email": Number(countNew("Your email")),
      "Confirm &amp; pay": Number(countNew("Confirm &amp; pay")),
      "Back to shows": Number(countNew("Back to shows")),
    };
    obs.feedback.includeEditedOnce =
      readFileSync(join(REPO, "design", "components", "Header.html"), "utf8").includes(
        "Help center",
      ) && Number(countNew("Help center")) === 1;
    ...
    for (const f of (await api.get(designer, `/projects/${PROJECT}/flows`)).json?.flows ?? [])
      if (f.pendingEdits > 0 && !obs.feedback.apply[f.id]) {
        ...
        obs.feedback.apply[f.id] = {
          ok: a.ok,
          applied: a.out?.applied ?? null,
          needsManual: a.out?.needsManual?.length ?? null,
          sourceFiles: (a.out?.results ?? []).reduce(
            (n, r) =>
              n + (r.files ?? []).filter((x) => x.result === "applied" && !x.include).length,
            0,
          ),
        };
      }
      ...
      firstIsAgent:
        Array.isArray(items) && items.length
          ? !!(
              items[0].sentToAgent ||
              items[0].agent ||
              /agent/i.test(JSON.stringify(items[0]).slice(0, 200))
            )
          : null,
      raw: JSON.stringify(dg.out).slice(0, 800),
    };
    ...
    if (agentId) {
      const rp = await c.call("portal_reply", {
        flow: "sign-up",
        thread: agentId,
        text: "Made the button label 'Continue' once; the second one was the footer link.",
      });
    ...
    await c.close();
    sh(
      "git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm feedback && git push -q",
    );
  });
...
if (!ROUNDS)
  await timed("publish2", async () => {
    const touch = (p, from, to) =>
      writeFileSync(join(REPO, p), readFileSync(join(REPO, p), "utf8").replace(from, to));
    ...
    Object.assign(obs.publish2, {
      ok: p.ok,
      pushed: p.out?.pushed?.map((x) => x.flow) ?? [],
      unchanged: p.out?.unchanged?.length ?? null,
      skipped: p.out?.skipped ?? null,
      components: p.out?.components?.state ?? null,
      release: p.out?.release?.number ?? null,
    });
    const rel = await api.get(designer, `/projects/${PROJECT}/releases`);
    const latest = (rel.json?.releases ?? [])[0];
    const detail = latest
      ? await api.get(designer, `/projects/${PROJECT}/releases/${latest.number}`)
      : null;
    const changes = detail?.json?.changes ?? [];
    const rows = changes.flatMap((c) => (c.screens ?? []).map((r) => ({ ...r, flow: c.id })));
    obs.publish2.diff = {
      rows: rows.length,
      viaInclude: rows.filter((r) => r.via === "include").length,
      viaSource: rows.filter((r) => r.via === "source").length,
      flowsChanged: changes.filter((c) => c.status === "changed").map((c) => c.id),
      flowsUnchanged: changes.filter((c) => c.status === "unchanged").length,
      sample: rows.filter((r) => r.via === "source").slice(0, 20),
    };
    ...
    await api.post(client, `/projects/${PROJECT}/flows/sign-up/comments`, {
      text: "One more: the welcome copy feels flat.",
      flowVersion: su.number,
    });
    ...
    obs.publish2.stale = {
      ...
      mentionsPull: /pull/i.test(stale.error?.message ?? ""),
      forceOffered: /force/i.test(stale.error?.message ?? ""),
    };
    ...
    await c.close();
    sh(
      "git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm round2 && git push -q",
    );
  });
...
if (!ROUNDS)
  await timed("handoff", async () => {
    ...
    for (const k of key.flows) {
      const h = await c.call("handoff", {
        flow: k.slug,
        story: `As a user I can ${k.title.toLowerCase()}`,
      });
      const entry = {
        slug: k.slug,
        ok: h.ok,
      ...
      if (h.ok) {
        const list =
          (await api.get(dev, `/projects/${PROJECT}/flows/${k.slug}/handoffs`)).json?.handoffs ??
          [];
        const id = list[0]?.id ?? h.out?.id;
        const full = id
          ? (await api.get(dev, `/projects/${PROJECT}/flows/${k.slug}/handoffs/${id}`)).json
          : null;
        const spec = full?.spec ?? "";
        entry.spec = {
          chars: spec.length,
          steps: k.steps.filter((s) => spec.includes(`## ${s.n} `)).length,
          of: k.steps.length,
          gridStates: full?.grid ? Object.keys(full.grid.states ?? full.grid).length : null,
          copyHits: ["Continue", "Pay COP 180.000", "Confirm and pay", "Back to events"].filter(
            (t) => spec.includes(t),
          ).length,
          transitions: k.transitions.filter((t) => spec.includes(t.to)).length,
          of_t: k.transitions.length,
          waived: /waiv|n\/a:/i.test(spec),
          screens: full?.screens?.length ?? null,
          devGet: !!full,
        };
      }
...
if (!SKIP.has("concurrency") && !ROUNDS)
  await timed("concurrency", async () => {
    ...
    writeFileSync(
      join(a, "design/flows/notifications/01-details-default.html"),
      readFileSync(join(a, "design/flows/notifications/01-details-default.html"), "utf8").replace(
        "Continue",
        "Next: channels",
      ),
    );
    writeFileSync(
      join(b, "design/flows/notifications/01-details-default.html"),
      readFileSync(join(b, "design/flows/notifications/01-details-default.html"), "utf8").replace(
        "Continue",
        "Go on",
      ),
    );
    const ca = mcp(a),
      cb = mcp(b);
    const before =
      (await api.get(designer, `/projects/${PROJECT}/flows/notifications/versions`)).json?.versions
        ?.length ?? 0;
    const [ra, rb] = await Promise.all([
      ca.call("publish", { note: "race A", flows: ["notifications"] }),
      cb.call("publish", { note: "race B", flows: ["notifications"] }),
    ]);
    await ca.close();
    await cb.close();
    const after =
      (await api.get(designer, `/projects/${PROJECT}/flows/notifications/versions`)).json?.versions
        ?.length ?? 0;
    obs.concurrency = {
      a: { ok: ra.ok, code: ra.error?.code ?? null },
      b: { ok: rb.ok, code: rb.error?.code ?? null },
      newVersions: after - before,
      clean: (ra.ok ? 1 : 0) + (rb.ok ? 1 : 0) === after - before,
    };
...
await timed("reliability", async () => {
  ...
  obs.reliability = {
    status: {
      ok: st.out?.ok ?? null,
      pluginNotice: st.out?.plugin?.message ?? null,
      latest: st.out?.plugin?.latest ?? null,
      blockers: st.out?.blockers?.map((b) => b.code) ?? [],
      nextSteps: st.out?.nextSteps?.length ?? null,
    },
  };
  ...
  obs.reliability.diagnose = {
    failedWithRun: !!run,
    code: bad.error?.code ?? null,
    linesForRun: dg?.out?.lines?.length ?? null,
  };
...
if (!SKIP.has("ui"))
  await timed("ui", async () => {
    ...
    const created = await api.post(admin, "/admin/clients", {
      email,
      name: "Suite Client",
      projectIds: [PROJECT],
    });
    ...
    if (inviteToken) {
      const password =
        "suite-" + Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2);
      const acc = await api.post(null, `/invites/${inviteToken}/accept`, {
        password,
        name: "Suite Client",
      });
      const login = await fetch(`${PORTAL}/api/v1/auth/password`, {
        method: "POST",
        headers: { "content-type": "application/json" },
    ...
    obs.ui = await uiChecks({
      api,
      portal: PORTAL,
      project: PROJECT,
      staffSession: await api.session(admin),
      clientSession,
      results: RESULTS,
