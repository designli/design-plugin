...
export async function runRounds(ctx) {
  ...
  const countNew = (text) =>
    Number(
      sh(
        `grep -rl --exclude-dir=bundle --include='*.html' -- ${JSON.stringify(text)} design | wc -l`,
      ).trim(),
    );
  const commit = (m) =>
    sh(
      `git add -A && git -c user.email=suite@designli.co -c user.name=Suite commit -qm ${JSON.stringify(m)} -q || true; git push -q`,
    );
  obs.rounds = {};
  ...
  await timed("journey", async () => {
    ...
    obs.journey = {
      ...
      expected,
      loop:
        nextOf("my-tickets").includes("transfer-a-ticket") &&
        nextOf("transfer-a-ticket").includes("my-tickets"),
      flowsOnPortal: flows.length,
    };
    obs.dedupe = {
      distinctScreens: key.dedupe?.distinctScreens ?? null,
      totalScreens: key.dedupe?.totalScreens ?? null,
      portalBytes: null,
    };
    note(
      `journey: ${connectors} connectors on the portal, ${expected} expected, loop ${obs.journey.loop}`,
    );
  });
  ...
  await timed("roundA", async () => {
    ...
    for (const f of twelve) {
      ...
      const r1 = await comment(f.slug, {
        text: `${f.title}: the primary action needs more contrast on this screen.`,
        screen: { id: s0.id, device: dev0 },
        anchor: { x: 60, y: 70 },
      ...
      if (mob)
        await comment(f.slug, {
          text: `On the phone the ${mob.title} header wraps to three lines.`,
          screen: { id: mob.id, device: "mobile" },
          anchor: { x: 50, y: 20 },
          flowVersion: v.number,
        });
      else
        await comment(f.slug, {
          text: `Overall ${f.title.toLowerCase()} reads well; the order of the steps makes sense.`,
          flowVersion: v.number,
        });
    }
    const bt = await latest("buy-tickets");
    await comment("buy-tickets", {
      text: "The map shows Seats before Tickets; should it not be the other way round?",
      screen: { id: "Main", device: "desktop" },
      anchor: { x: 30, y: 30 },
      flowVersion: bt.number,
    });
    // on the screen release 2 removes: this thread must survive as an orphan of v1
    const rm = R.release2.removedStep;
    await comment(rm.flow, {
      text: "Can the row picker show the price per row?",
      screen: { id: `${rm.step}-${rm.id}-Default`, device: "desktop" },
      anchor: { x: 20, y: 30 },
      flowVersion: (await latest(rm.flow)).number,
    });
    ...
    const md = await comment("sign-up", {
      text: '**Bold claim**: this works. <img src=x onerror="alert(1)"> `code` and a [link](https://example.com)',
      screen: { id: "01-Email-Default", device: "desktop" },
      anchor: { x: 10, y: 10 },
      flowVersion: 1,
    });
    A.comments.xssStoredVerbatim = md.json?.text?.includes("onerror") ?? null;
    const es = await comment("find-an-event", {
      text: "¿Podría el botón decir «Buscar eventos» en lugar de «Buscar»? 🎟️ Gracias, se ve muy bien.",
      flowVersion: 1,
    });
    ...
    const edit = async (t, elementPath) => {
      ...
      const r = await api.post(client, `${flowOf(t.flow)}/text-edits`, {
        screen: { id: t.screen, device },
        elementPath,
        originalText: t.text,
        originalHash: textHash(t.text),
        newText: t.newText,
        flowVersion: v.number,
      });
      A.edits.posted++;
    ...
    const ordered = [...R.A.editTargets].sort(
      (a, b) => (a.supersededBy ? 0 : 1) - (b.supersededBy ? 0 : 1),
    );
    const replacers = new Set(
      R.A.editTargets.filter((t) => t.supersededBy).map((t) => t.supersededBy),
    );
    ...
    await edit(
      {
        name: "dismiss1",
        flow: "sign-up",
        screen: "03-Welcome-Default",
        text: "Back to events",
        newText: "Back to shows",
      },
      "b/1/90",
    );
    await edit(
    ...
    for (const f of key.flows) {
      ...
      for (const m of grid?.missing ?? []) {
        if (waived >= 3) break;
        const r = await api.post(designer, `${flowOf(f.slug)}/waivers`, {
          step: m.step,
          state: m.state,
          reason: `pilot scope: ${m.state.toLowerCase()} does not occur in ${f.slug}`,
        });
        if (r.status === 200) {
          waived++;
    ...
    const cw = await api.post(client, `${flowOf("request-a-refund")}/waivers`, {
      step: "02",
      state: "Error",
      reason: "client says so",
    });
    ...
    const tooLargeWaiver = await api.post(designer, `${flowOf("reports")}/waivers`, {
      step: "02",
      state: "Default",
      reason: "trying to waive a too-large state",
    });
    A.tooLarge = {
      flow: "reports",
      step: "02",
      gridStatus: reportsStep02?.states?.Default?.status ?? null,
      waiverStatus: tooLargeWaiver.status,
    };
    // structure edits from the portal side (a client cannot; staff can): step titles and an entry point
    const st1 = await api.put(designer, `${flowOf("sign-up")}/structure`, {
      waivers: {},
      stepTitles: { "01": "Your email" },
      entryPoints: [{ from: "Landing page", to: "01-Email" }],
    });
    const st2 = await api.put(designer, `${flowOf("wallet")}/structure`, {
      waivers: {},
      stepTitles: { "01": "Your balance" },
    });
    const stc = await api.put(client, `${flowOf("sign-up")}/structure`, {
      waivers: {},
      stepTitles: { "01": "Nope" },
    });
    A.structure = {
      stepTitles: 2,
      entryPoints: 1,
      status: [st1.status, st2.status],
      clientRefused: stc.status === 403,
    };
    // the journey reordered
    const all = (await api.get(designer, `/projects/${PROJECT}/flows`)).json.flows.map((f) => f.id);
    const ro = await api.put(designer, `/projects/${PROJECT}/flows/order`, {
      ids: [...all].reverse(),
    });
    ...
    for (const t of firstThreads.slice(0, 2)) {
      const r = await api.patch(designer, `${flowOf(t.flow)}/comments/${t.id}`, {
        sentToAgent: true,
      });
    ...
    if (t3) {
      ...
      const open = (
        await api.get(designer, `${flowOf(t3.flow)}/comments?status=open`)
      ).json?.threads?.some((x) => x.id === t3.id);
      A.reopened = {
        resolveStatus: res.status,
        reopenStatus: reo.status,
        statusAfter: open ? "open" : "resolved",
      };
    }
    note(
      `round A: waivers ${waived}, structure ${A.structure.status.join("/")}, reorder ${A.reorder}, agent threads ${A.agentThreads.length}, reopen ${A.reopened.statusAfter}`,
    );
  });
  ...
  await timed("release2", async () => {
    ...
    for (const name of ["dismiss1", "dismiss2"]) {
      ...
      if (id) {
        const d = await c.call("edits_dismiss", {
          flow,
          id,
          reason: "the label matches the button on the previous screen; keeping it",
        });
    ...
    for (const t of R.A.editTargets)
      if (t.filesTouched !== undefined)
        r2.editsLanded[t.name] = countNew(t.newText.replace(/&/g, "&amp;"));
    r2.editsLanded.dismiss1 = countNew("Back to shows");
    for (const t of obs.rounds.A.agentThreads) {
      const rp = await c.call("portal_reply", {
        flow: t.flow,
        thread: t.id,
        text: "Done in round 2: contrast raised on the primary action.",
      });
      const rs = await c.call("portal_resolve", { flow: t.flow, thread: t.id });
      if (rp.ok && rs.ok) r2.replies++;
    }
    note(
      `release 2: applied ${r2.applied}, needsManual ${r2.needsManual}, dismissed ${r2.dismissed}, replies ${r2.replies}, landed ${JSON.stringify(r2.editsLanded)}`,
    );
    // the designer's changes
    const fdir = (s) => join(REPO, "design", "flows", s);
    const rm = R.release2.removedStep;
    for (const f of readdirSync(fdir(rm.flow)))
      if (f.startsWith(`${rm.step}-`)) rmSync(join(fdir(rm.flow), f));
    const sm = JSON.parse(readFileSync(join(fdir(rm.flow), "flow.json"), "utf8"));
    sm.steps = sm.steps.filter((s) => s.n !== rm.step);
    sm.transitions = (sm.transitions ?? []).filter(
      (t) => !t.from.startsWith(`${rm.step}-`) && !t.to.startsWith(`${rm.step}-`),
    );
    ...
    for (const f of src) {
      ...
      writeFileSync(
        join(wdir, target),
        readFileSync(join(wdir, f), "utf8")
          .replace(
            /<h1>[^<]*<\/h1>/,
            `<h1>Cards · ${target.split("-")[2].replace(/(-m)?\.html$/, "")}</h1>`,
          )
          .replace("Phone", "Card nickname"),
      );
      const state = target
        .split("-")[2]
        .replace(/(-m)?\.html$/, "")
    ...
    for (const f of readdirSync(bdir))
      if (fromRe.test(f))
        sh(
          `git mv ${JSON.stringify(f)} ${JSON.stringify(f.replace(fromRe, rn.to.toLowerCase()))}`,
          bdir,
        );
    // the state key keeps the designer's casing, the file paths stay lower case
    const bj = readFileSync(join(bdir, "flow.json"), "utf8").replace(fromRe, (m) =>
      m === m.toLowerCase() ? rn.to.toLowerCase() : rn.to,
    );
    ...
    let p = await c.call("publish", {
      note: "Release 2: seat picking simplified, saved cards, ThreeDS rename, footer copy",
    });
    if (!p.ok && p.error?.code === "STALE_LOCAL") {
      await tool(c, "feedback_pull", {});
      p = await tool(c, "publish", {
        note: "Release 2: seat picking simplified, saved cards, ThreeDS rename, footer copy",
      });
    } else if (!p.ok) ctx.fail("release2 publish", p.error);
    r2.publish = {
      ok: p.ok,
      pushed: p.out?.pushed?.map((x) => x.flow) ?? [],
      unchanged: p.out?.unchanged?.length ?? null,
      skipped: p.out?.skipped?.map((x) => x.flow) ?? [],
      release: p.out?.release?.number ?? null,
      error: p.error ? `${p.error.code}: ${p.error.message?.slice(0, 200)}` : null,
    };
    ...
    await c.close();
    note(
      `release 2 published: ${r2.publish.pushed.length} pushed, ${r2.publish.unchanged} unchanged, skipped ${r2.publish.skipped.join(",") || "none"}${r2.publish.error ? "; error " + r2.publish.error : ""}; portalOnly ${r2.portalOnly.join(",") || "none"}, archived ${r2.archived.ok}/${r2.portalOnly.length}`,
    );
    // what the portal says about it
    const det = r2.publish.release
      ? (await api.get(designer, `/projects/${PROJECT}/releases/${r2.publish.release}`)).json
      : null;
    ...
    r2.diff = {
      ...
      changedSource: rows.filter((r) => r.status === "changed" && r.via === "source").length,
      changedInclude: rows.filter((r) => r.via === "include").length,
      byFlow: {
        seatMapRemoved: by(rm.flow, "removed"),
        walletAdded: by(add.flow, "added"),
        buyTicketsRenamed: { removed: by(rn.flow, "removed"), added: by(rn.flow, "added") },
      },
      flowsChanged: changes.filter((c) => c.status === "changed").map((c) => c.id),
    };
    const orphanScreen = `${rm.step}-${rm.id}-Default`;
    const th =
      (await api.get(designer, `${flowOf(rm.flow)}/comments?status=all&screen=${orphanScreen}`))
        .json?.threads ?? [];
    const v1 = (await api.get(designer, `${flowOf(rm.flow)}/versions/1`)).json;
    r2.orphan = {
      threadsOnRemovedScreen: th.length,
      readableViaVersionsApi: !!v1?.manifest?.screens?.some((s) => s.id === orphanScreen),
      screen: orphanScreen,
    };
    ...
    r2.deletedFlow = {
      stillOnPortal: !!del,
      latestVersionUnchanged: del ? del.latestVersion === before[del.id] : null,
    };
    r2.renamedFlow = {
      newOnPortal: after.some((f) => f.id === rf.to),
      oldStillOnPortal: after.some((f) => f.id === rf.from),
    };
    const grid = (await api.get(designer, `${flowOf("request-a-refund")}/states`)).json;
    r2.waiversSurvive = !!grid?.steps?.some((s) =>
      Object.values(s.states ?? {}).some((x) => x.status === "waived"),
    );
    const head = (await api.get(designer, `${flowOf("sign-up")}/head`)).json;
    r2.stepTitleKept = head?.structure?.stepTitles?.["01"] ? "portal" : "lost";
    r2.stepTitleInManifest =
      (await latest("sign-up"))?.manifest?.steps?.find((s) => s.n === "01")?.title ?? null;
    note(
  ...
  await timed("roundB", async () => {
    ...
    const r = await api.post(client, `${flowOf(se.flow)}/text-edits`, {
      screen: { id: se.screen, device: "desktop" },
      elementPath: "b/1/95",
      originalText: se.text,
      originalHash: textHash(se.text),
      newText: "E-mail",
      flowVersion: 1,
    });
    void v;
    ...
    B.staleEdit = {
      posted: r.status,
      result: mine ? (mine.result === "applied" ? "applied" : "needsManual") : "not-found",
      detail: mine?.result ?? null,
    };
    if (B.staleEdit.result === "needsManual") {
      ...
      const openThreads = (
        await api.get(designer, `${flowOf(se.flow)}/comments?status=open&screen=${se.screen}`)
      ).json;
      const threadFound = (openThreads?.threads ?? []).some((t) =>
        /changed in version/i.test(t.text),
      );
      B.staleEdit.outdated = { suggested, status: edit?.status ?? null, threadFound };
    }
    await c.close();
    const oc = R.B.orphanComment;
    const o = await api.post(client, `${flowOf(oc.flow)}/comments`, {
      text: "Still thinking about the row picker we had here.",
      screen: { id: oc.screen, device: "desktop" },
      anchor: { x: 20, y: 20 },
      flowVersion: oc.version,
    });
    B.orphanComment = { status: o.status, code: o.code };
    const wp = R.B.waiverOnPresent;
    const w = await api.post(designer, `${flowOf(wp.flow)}/waivers`, {
      step: wp.step,
      state: wp.state,
      reason: "trying to waive a designed state",
    });
    ...
    if (cards)
      await api.post(client, `${flowOf("wallet")}/comments`, {
        text: "Saved cards: can I set a default?",
        screen: { id: cards.id, device: cards.devices.desktop ? "desktop" : "mobile" },
        anchor: { x: 40, y: 40 },
        flowVersion: wallet.number,
      });
    note(
      `round B: stale edit → ${B.staleEdit.result} (${B.staleEdit.detail}), orphan comment ${o.status}, waiver on a present state ${w.status} (state ${st})`,
    );
  });
  ...
  await timed("release3", async () => {
    ...
    for (const f of (await api.get(designer, `/projects/${PROJECT}/flows`)).json?.flows ?? [])
      if (f.pendingEdits > 0) await c.call("edits_apply", { flow: f.id });
    ...
    for (const slug of R.release3.subset) {
      const f =
        readdirSync(join(REPO, "design", "flows", slug)).find(
          (x) => /-default\.html$/.test(x) && !/-m\.html$/.test(x),
        ) ?? readdirSync(join(REPO, "design", "flows", slug)).find((x) => x.endsWith(".html"));
    ...
    const versionsBefore = Object.fromEntries(
      ((await api.get(designer, `/projects/${PROJECT}/flows`)).json?.flows ?? []).map((f) => [
        f.id,
        f.latestVersion,
      ]),
    );
    const sub = await tool(c, "publish", {
      note: "Release 3a: five flows only",
      flows: R.release3.subset,
    });
    ...
    r3.subset = {
      pushed: sub.out?.pushed?.map((x) => x.flow) ?? [],
      othersUntouched: Object.keys(versionsBefore)
        .filter((s) => !R.release3.subset.includes(s))
        .every((s) => mid[s] === versionsBefore[s]),
      release: sub.out?.release?.number ?? null,
    };
    const full = await tool(c, "publish", { note: "Release 3b: everything else" });
    r3.full = {
      pushed: full.out?.pushed?.map((x) => x.flow) ?? [],
      release: full.out?.release?.number ?? null,
    };
    ...
    const firstNow = first
      ? (await api.get(dev, `${flowOf("buy-tickets")}/handoffs/${first.id}`)).json
      : null;
    r3.handoffs = {
      v1StillV1: firstNow ? firstNow.version === first.version : null,
      v3Created: h.ok && h2.length === h1.length + 1,
      v1ScreensUrlsHaveV1: firstNow
        ? (firstNow.screens ?? []).every((s) =>
            Object.values(s.devices).every((d) => d.url.includes(`/v${first.version}/`)),
          )
        : null,
      firstVersion: first?.version ?? null,
      latestVersion: (await latest("buy-tickets"))?.number ?? null,
    };
    await c.close();
    note(
      `release 3: subset pushed ${r3.subset.pushed.join(",")} (others untouched ${r3.subset.othersUntouched}); full pushed ${r3.full.pushed.length}; handoff v${r3.handoffs.firstVersion} frozen ${r3.handoffs.v1StillV1}, new handoff ${r3.handoffs.v3Created}`,
    );
  });
  ...
  await timed("release4", async () => {
    ...
    await api.post(client, `${flowOf("sign-in")}/comments`, {
      text: "The code field should accept paste.",
      flowVersion: v?.number ?? null,
    });
    ...
    r4.forced = {
      refusedFirst: !refused.ok && refused.error?.code === "STALE_LOCAL",
      ok: forced.ok,
      release: forced.out?.release?.number ?? null,
    };
    ...
    const releasesBefore =
      (await api.get(designer, `/projects/${PROJECT}/releases`)).json?.releases?.length ?? null;
    ...
    r4.noop = {
      ok: noop.ok,
      noop: noop.out?.noop === true,
      release: noop.out?.release ?? null,
      releasesBefore,
      releasesAfter,
      calls: httpAfter - httpBefore,
    };
    await c.close();
    note(
      `release 4: refused first ${r4.forced.refusedFirst}, forced ${r4.forced.ok} (release ${r4.forced.release}); no-op publish → noop ${r4.noop.noop}, releases ${r4.noop.releasesBefore} → ${r4.noop.releasesAfter}`,
    );
  });
  ...
  await timed("mcp", async () => {
    ...
    const hid =
      lh.result?.structuredContent?.handoffs?.[0]?.id ??
      lh.result?.handoffs?.[0]?.id ??
      (typeof lh.result?.content?.[0]?.text === "string"
        ? (JSON.parse(lh.result.content[0].text).handoffs ?? [])[0]?.id
        : undefined);
    const gh = await devMcp.call("get_handoff", {
      project: PROJECT,
      flow: "buy-tickets",
      ...(hid ? { id: hid } : {}),
    });
    const gg = await devMcp.call("get_states_grid", { project: PROJECT, flow: "buy-tickets" });
    const gr = await devMcp.call("get_release", {
      project: PROJECT,
      number: obs.rounds.release3?.full?.release ?? 1,
    });
    const status = (r) =>
      r.error
        ? `rpc:${r.error.code ?? r.error.message}`
        : r.isError
          ? "tool-error"
          : r.ok
            ? "ok"
    ...
    const pf = await clientMcp.call("push_flow_version", {
      project: PROJECT,
      flow: "sign-in",
      bundle: { manifest: {}, files: [] },
      ifMatch: 1,
    });
    const ws = await clientMcp.call("waive_state", {
      project: PROJECT,
      flow: "request-a-refund",
      step: "02",
      state: "Error",
      reason: "no",
    });
    ...
    obs.mcp = {
      devAgent: {
        listHandoffs: status(lh),
        getHandoff: status(gh),
        getStatesGrid: status(gg),
        getRelease: status(gr),
      },
      clientRefused: {
        pushFlowVersion: pf.isError || !pf.ok ? (pf.error?.code ?? "FORBIDDEN") : null,
        waiveState: ws.isError || !ws.ok ? (ws.error?.code ?? "FORBIDDEN") : null,
      },
      tokenLeaked: /dpat_[A-Za-z0-9_-]{8,}/.test(blob),
      sample: (gh.result?.content?.[0]?.text ?? "").slice(0, 200),
    };
    note(
      `mcp: dev ${JSON.stringify(obs.mcp.devAgent)}, client ${JSON.stringify(obs.mcp.clientRefused)}`,
    );
  });
  ...
  await timed("concurrency3", async () => {
    ...
    clones.forEach((d, i) => {
      ...
      if (f)
        writeFileSync(
          join(dir, f),
          readFileSync(join(dir, f), "utf8").replace("<h1>", `<h1>C3-${i} · `),
        );
    });
    ...
    const rs = await Promise.all(
      cs.map((c, i) => c.call("publish", { note: `race ${i}`, flows: [targets[i]] })),
    );
    ...
    obs.concurrency = {
      ...
      threeFlows: {
        ...
        conflicts: rs.filter((r) => !r.ok).length,
        codes: rs.map((r) => r.error?.code ?? "ok"),
        newVersions,
      },
    };
    for (const d of clones) rmSync(d, { recursive: true, force: true });
    note(
      `concurrency: three flows at once → ${obs.concurrency.threeFlows.ok} ok, ${obs.concurrency.threeFlows.conflicts} conflicts, ${newVersions} new versions`,
    );
  });

  writeFileSync(
    join(REPO, "NOTES.md"),
    `# Marquee XL on ${PORTAL}\n\nProject \`${PROJECT}\`. What the suite did, in order:\n\n${notes.join("\n")}\n`,
  );
  commit("notes");
  ...
  const bothArchived =
    obs.rounds.release2?.deletedFlow?.stillOnPortal === false &&
    obs.rounds.release2?.renamedFlow?.oldStillOnPortal === false;
  const extraEdges =
    !bothArchived &&
    obs.rounds.release2?.renamedFlow?.oldStillOnPortal &&
    obs.rounds.release2?.renamedFlow?.newOnPortal
      ? (oldFlow?.next ?? []).length
      : 0;
  const expectedBase = bothArchived
    ? key.flows.reduce(
        (n, f) =>
          f.slug === deletedSlug
            ? n
            : n + (f.next ?? []).filter((l) => l.flow !== deletedSlug).length,
        0,
      )
    : obs.journey?.expected;
  const ui = {
    journeyExpected: expectedBase != null ? expectedBase + extraEdges : null,
    orphan: obs.rounds.release2?.orphan
      ? {
          flow: R.release2.removedStep.flow,
          screen: obs.rounds.release2.orphan.screen,
          version: 1,
          text: "row picker",
        }
      : null,
    release2: obs.rounds.release2?.publish?.release ?? null,
    sheets: (key.components?.files ?? []).length,
  };
  return ui;
}
