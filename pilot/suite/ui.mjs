...
export async function uiChecks({
  portal,
  project,
  staffSession,
  clientSession,
  ...
  const visit = async (ctx, path, name, { shot = false, wait = 1500, onPage = null } = {}) => {
    ...
    const res = await page
      .goto(portal + path, { waitUntil: "networkidle", timeout: 45000 })
    ...
    if (shot) {
      await page
        .screenshot({ path: join(shots, `${name}.png`), fullPage: false })
        .catch(() => null);
  ...
  try {
    ...
    try {
      ...
      out.checks.journey = {
        connectors: edges || labelLinks,
        loops: loopLinks,
        expected: journeyExpected ?? null,
      };
    } catch (e) {
    ...
    for (const slug of flows) {
      page = await visit(d, `/projects/${project}/flows/${slug}`, `flow-${slug}`, {
        shot: true,
        wait: 2500,
      });
      ...
      const mobileDisabled = (await mobileBtn.count())
        ? await mobileBtn.isDisabled().catch(() => null)
        : null;
    ...
    if (orphan) {
      try {
        ...
        const op = await visit(
          d,
          `/projects/${project}/flows/${orphan.flow}?v=${orphan.version}&tab=comments`,
          `orphan-${orphan.flow}`,
          { shot: true, wait: 2500 },
        );
        const text = await op.locator("body").innerText();
        const screenListed = text.includes(orphan.screen);
        const threadListed = orphan.text
          ? text.toLowerCase().includes(String(orphan.text).toLowerCase())
          : null;
        const tab = await op.locator("[data-testid='comments-tab']").count();
        out.checks.orphanVersion = {
          visible: tab > 0 && (threadListed ?? screenListed),
          screenListed,
          threadListed,
        };
        await op.close();
    ...
    page = await visit(d, `/projects/${project}?tab=components`, "components", {
      shot: true,
      wait: 2500,
    });
    ...
    if (release2 != null) {
      try {
        const rp = await visit(d, `/projects/${project}?tab=releases&r=${release2}`, "release2", {
          shot: true,
          wait: 2000,
        });
        ...
        out.checks.release2 = {
          rows,
          viaIncludeShown: /\binclude\b/i.test(text) || /\bthrough\b/i.test(text),
        };
        await rp.close();
      } catch (e) {
    ...
    out.checks.deviceBadCode = /start the sign-in again|expire|not found|no pending/i.test(
    ...
    out.checks.client = {
      adminLink: /\bAdmin\b/.test(text),
      accountLink: /Account and tokens/.test(text),
      sees: text.slice(0, 200),
    };
    await page.close();
